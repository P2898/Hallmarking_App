          ...
          <View className="flex-row flex-wrap gap-2">
            {categories.map(cat => (
              <TouchableOpacity 
                ...
                className={`px-4 py-2 rounded-full border ${selectedCategories.includes(cat.name) ? 'bg-gold border-gold' : 'bg-white border-gray-300'}`}
              >
                <Text className={selectedCategories.includes(cat.name) ? 'text-white font-semibold' : 'text-gray-600'}>
                  {cat.name}
                </Text>
              </TouchableOpacity>
            ))}
