...
async function authenticate() {
    ...
    try {
        const cred = await (0, auth_1.createUserWithEmailAndPassword)(auth, email, password);
        console.log(`✅ Authenticated successfully as temp user: ${cred.user.uid}`);
        return cred.user;
    }
    catch (error) {
        if (error.code === 'auth/email-already-in-use') {
            const cred = await (0, auth_1.signInWithEmailAndPassword)(auth, email, password);
...
async function runMigration() {
    try {
        ...
        const hashedDefaultPassword = await bcryptjs_1.default.hash(defaultPassword, salt);
        console.log('\n📥 Fetching Categories from Firestore...');
        const catSnapshot = await (0, firestore_1.getDocs)((0, firestore_1.collection)(firestore, 'categories'));
        console.log(`Found ${catSnapshot.size} categories.`);
        for (const doc of catSnapshot.docs) {
            const data = doc.data();
            await models_1.Category.findOrCreate({
                where: { id: doc.id },
                defaults: {
                    id: doc.id,
        ...
        const userSnapshot = await (0, firestore_1.getDocs)((0, firestore_1.collection)(firestore, 'users'));
        ...
        const listingSnapshot = await (0, firestore_1.getDocs)((0, firestore_1.collection)(firestore, 'listings'));
        console.log(`Found ${listingSnapshot.size} listings.`);
        for (const doc of listingSnapshot.docs) {
            const data = doc.data();
            let categoryId = data.category;
            if (categoryId) {
                const catExists = await models_1.Category.findByPk(categoryId);
                if (!catExists) {
                    const firstCat = await models_1.Category.findOne();
                    categoryId = firstCat ? firstCat.id : null;
                }
            }
            let sellerId = data.userId;
            if (sellerId) {
                const userExists = await models_1.User.findByPk(sellerId);
                if (!userExists) {
            ...
            await models_1.Listing.findOrCreate({
                where: { id: doc.id },
                defaults: {
                    id: doc.id,
        ...
        const chatSnapshot = await (0, firestore_1.getDocs)((0, firestore_1.collection)(firestore, 'chats'));
        ...
        for (const chatDoc of chatSnapshot.docs) {
            ...
            const listingExists = await models_1.Listing.findByPk(listingId);
            const buyerExists = await models_1.User.findByPk(buyerId);
            const sellerExists = await models_1.User.findByPk(sellerId);
            if (!listingExists || !buyerExists || !sellerExists) {
                console.log(`⚠️ Skipping chat ${chatDoc.id} because referenced entities don't exist.`);
                continue;
            }
            await models_1.Chat.findOrCreate({
                where: { id: chatDoc.id },
                defaults: {
                    id: chatDoc.id,
                    buyerId,
            ...
            const msgSnapshot = await (0, firestore_1.getDocs)((0, firestore_1.collection)(firestore, 'chats', chatDoc.id, 'messages'));
            for (const msgDoc of msgSnapshot.docs) {
                const msgData = msgDoc.data();
                await models_1.Message.findOrCreate({
                    where: { id: msgDoc.id },
                    defaults: {
                        id: msgDoc.id,
                        chatId: chatDoc.id,
            ...
            const offerSnapshot = await (0, firestore_1.getDocs)((0, firestore_1.collection)(firestore, 'chats', chatDoc.id, 'offers'));
            for (const offerDoc of offerSnapshot.docs) {
                const offerData = offerDoc.data();
                await models_1.Offer.findOrCreate({
