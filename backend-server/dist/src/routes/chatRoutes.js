...
router.get('/', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        const chats = await models_1.Chat.findAll({
            where: {
                [sequelize_1.Op.or]: [{ buyerId: userId }, { sellerId: userId }],
            },
            include: [
                { model: models_1.User, as: 'buyer', attributes: ['id', 'email', 'displayName', 'photoURL', 'companyName', 'city', 'state'] },
                { model: models_1.User, as: 'seller', attributes: ['id', 'email', 'displayName', 'photoURL', 'companyName', 'city', 'state'] },
...
router.get('/all', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
router.post('/', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        if (!chat) {
            ...
            chat = await models_1.Chat.findByPk(chat.id, {
                include: [
                    { model: models_1.User, as: 'buyer', attributes: ['id', 'email', 'displayName', 'photoURL', 'companyName', 'city', 'state'] },
                    { model: models_1.User, as: 'seller', attributes: ['id', 'email', 'displayName', 'photoURL', 'companyName', 'city', 'state'] },
                    { model: models_1.Listing, as: 'listing', attributes: ['id', 'title', 'price', 'images'] },
...
router.get('/:chatId/messages', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const chatId = req.params.chatId;
        const userId = req.user.id;
        // Check if user belongs to this chat
        const chat = await models_1.Chat.findByPk(chatId);
...
// POST a message
router.post('/:chatId/messages', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const chatId = req.params.chatId;
        const { text } = req.body;
...
router.get('/:chatId/offers', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const chatId = req.params.chatId;
        const userId = req.user.id;
        const chat = await models_1.Chat.findByPk(chatId);
        ...
        const offers = await models_1.Offer.findAll({
            where: { chatId },
            order: [['createdAt', 'ASC']],
        });
...
// POST an offer
router.post('/:chatId/offers', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const chatId = req.params.chatId;
...
router.put('/:chatId/offers/:offerId', authMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const chatId = req.params.chatId;
        ...
        const offer = await models_1.Offer.findOne({ where: { id: offerId, chatId } });
        if (!offer) {
